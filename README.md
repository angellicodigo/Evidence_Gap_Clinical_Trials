...
6. Download the weights from huggingface by typing this command
$ hf download nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-FP8
7. Kill the LSF job run
...
$ cd /sc/arion/projects/EHR_ML/$USER/Evidence_Gap_Clinical_Trials/jobs
2. Start a Ray cluster. If there is an port error, you have to change PORT number in the file. In your /sc/arion/scratch/$USER/ folder there will be a file called "ray_address" that consist the address of the Ray cluster. You can view the status of the cluster by doing $ ray status --address=<ray_address>
...
$ touch /sc/arion/scratch/$USER/run_code
